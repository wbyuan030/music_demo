...
impl MusicHandler {
    pub fn new(app_handle: AppHandle) -> Self {
        ...
        Self::spawn_handle_event(app_handle_event, event_receiver, handler_sink);
        Self::spawn_progress(app_progress_cloned, progress_sink);
        Self {
            event_sender,
            sink,
            stream_handle,
            ...
            while let Ok(event) = event_receiver.recv().await {
                match event {
                    MusicState::Play(id) => {
                        ...
                        let track_data = match parse_track_request(id, track).await {
                            Ok(data) => data,
                            Err(e) => {
                                error!("{}", e);
                                continue;
    ...
    fn spawn_progress(app_handle: AppHandle, sink: Arc<Mutex<Sink>>) {
        tokio::spawn(async move {
            // 播放状态
            ...
            loop {
                ...
                if sink.empty() {
                    ...
                    app_handle.emit("play_progress", sink.get_pos());
                    curr_state = true
                }
